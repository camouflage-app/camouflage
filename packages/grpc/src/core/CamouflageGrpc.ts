import { CamouflageGrpcConfig, CamouflageGrpcHook, Hooks, isConfigValid } from "./config.js";
import { log } from "./logger.js";
import path from 'path';
import fs from 'fs';
import * as grpc from "@grpc/grpc-js";
import { CamouflageGrpcHandler } from "../utils/handlers.js";
import { registerCustomHelpers } from "../helpers/index.js";
import * as client from 'prom-client';
import http from 'http';
import Helpers from "@camouflage/helpers";

export default class CamouflageGrpc {
    private config: CamouflageGrpcConfig | null = null
    private server: grpc.Server;
    private handlers: CamouflageGrpcHandler | undefined
    private helpers: Helpers;
    private hooks: Hooks = {};
    constructor(config?: CamouflageGrpcConfig) {
        if (config) {
            ...
            if (isValid) {
                ...
                this.handlers = new CamouflageGrpcHandler(this.config, this.hooks)
            } else {
    ...
    public loadConfigFromJson = (configFilePath: string): void => {
        ...
        if (fs.existsSync(absolutePath)) {
            const configData = JSON.parse(fs.readFileSync(absolutePath, 'utf-8')) as CamouflageGrpcConfig;
    ...
    public addHook = (fullPath: string, event: "onRequest" | "beforeResponse" | "afterResponse", fn: CamouflageGrpcHook): void => {
